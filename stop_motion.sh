...
newfile=`/bin/date +%Y%m%d%H_%M_%S`

array=(`ls $stream_movie_dir`)
len=${#array[*]}
if [[ $len -eq 1 ]]
then
    ffmpeg -i "$stream_movie_dir/$movie_timestamp$movie_format" -ss $movie_start_motion -t $movie_delay -c copy "$captured_movie_dir/$newfile$movie_format" 2>
...
<------>    ffmpeg -i "$stream_movie_dir/$movie$movie_format" -ss $movie_start_motion -c copy "$tmp_movie_dir/part1$movie_format" 2> /dev/null
<------>    ffmpeg -i "$stream_movie_dir/$movie1$movie_format" -ss 0 -t $period_to_sec_movie -c copy "$tmp_movie_dir/part2$movie_format" 2> /dev/null
...
<------>    ffmpeg -i "$stream_movie_dir/$movie1$movie_format" -ss $movie_start_motion -t $movie_delay -c copy "$captured_movie_dir/$newfile$movie_format" 2>
